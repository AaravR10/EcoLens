...
async function predict() {
    ...
    const recyclingData = CITY_RECYCLING[city];
    ...
    // Basic UI Setup
    resultTitle.textContent = topResult.className;

    if (recyclingData) {
        verdictNote.textContent = recyclingData.note;

        // Remove any existing wash info boxes to prevent duplicates
        const oldWashBox = document.querySelector('.wash-info-box');
        if (oldWashBox) oldWashBox.remove();

        // NEW: Add the Wash Mandate logic
        if (recyclingData.wash_mandate) {
            const washBox = document.createElement('div');
            washBox.className = 'wash-info-box';

            // Inline styling for quick integration
            washBox.style.marginTop = '15px';
            washBox.style.padding = '12px';
            washBox.style.borderLeft = '4px solid var(--amber)';
            washBox.style.backgroundColor = 'var(--gray-100)';
            washBox.style.borderRadius = 'var(--radius-sm)';

            washBox.innerHTML = `
                <div style="color: var(--amber); font-weight: 700; font-size: 13px; margin-bottom: 4px; text-transform: uppercase;">
                    ⚠️ ${recyclingData.wash_mandate} Cleaning Requirement
                </div>
                <div style="font-size: 14px; color: var(--gray-800);">
                    ${recyclingData.wash_note}
                </div>
            `;
            resultDetails.appendChild(washBox);
        }
    }

    // Save to history (Supabase/LocalStorage logic remains same)
    saveScanToHistory(topResult.className, city, recyclingData?.verdict || 'unknown');
}
function stopCamera() {
...
const CITY_RECYCLING = {
    seattle: {
        ...
        note: 'Robust curbside program; glass drop-off only.',
        wash_mandate: 'Mandatory',
        wash_note: 'Seattle Municipal Code requires all food and liquid containers to be empty and clean.'
    },
    ...
    portland: {
        verdict: 'yes',
        bin: 'Blue recycling bin',
        note: 'Strong curbside program',
        wash_mandate: 'Mandatory',
        wash_note: 'Standard Clean and Dry rule applies to avoid contaminating truck-loads.'
    },
    ...
    nyc: {
        verdict: 'yes',
        bin: 'Recycling bin',
        note: 'Broad curbside; no plastic bags',
        wash_mandate: 'Strict',
        wash_note: 'Must rinse all plastic containers, beverage cartons, and glass jars. Fines apply for food residue.'
    },
    ...
    austin: {
        verdict: 'yes',
        bin: 'Single-stream bin',
        note: 'No rinsing required',
        wash_mandate: 'Required',
        wash_note: 'Requires aluminum foil to be cleaned and plastic containers to be free of grease.'
    },
    ...
    chicago: {
        verdict: 'yes',
        bin: 'Blue Cart',
        note: 'Accepted curbside',
        wash_mandate: 'Required',
        wash_note: 'Items must be loose and free of food waste to avoid Gross Contamination tags.'
    },
    ...
    san_diego: {
        verdict: 'yes',
        bin: 'Blue bin',
        note: 'Single-stream curbside',
        wash_mandate: 'Required',
        wash_note: 'Official policy defines acceptable plastics as "clean," excluding significant food residue.'
    },
    ...
    philadelphia: {
        verdict: 'yes',
        bin: 'Blue bin',
        note: 'Single-stream; rinse required',
        wash_mandate: 'Required',
        wash_note: 'Code requires materials be free of food or excessive grease (wipe or rinse residue).'
    },
    ...
    minneapolis: {
        verdict: 'yes',
        bin: 'Blue bin',
        note: 'Organics + recycling program',
        wash_mandate: 'Required',
        wash_note: 'Food containers must be rinsed to protect the sorting facility workers.'
    },
    ...
    charlotte: {
        verdict: 'yes',
        bin: 'Blue bin',
        note: 'Single-stream program',
        wash_mandate: 'Required',
        wash_note: 'Food-soiled paper and un-rinsed plastic are considered contamination.'
    },
    ...
    boston: {
        ...
        note: 'Single-stream curbside',
        wash_mandate: 'Strict',
        wash_note: 'City regulations mandate all containers be empty, clean, and dry. Tickets may be issued.'
    },
